...
def make_conditioners(
    ...
    shape_info : ShapeInfo
        A ShapeInfo instance that knows the shapes of
    ...
    **kwargs :
    ...
    if _is_sin(activation):
        scale_first_weights = kwargs.pop("siren_scale_first_weights", True)
        initialize = kwargs.pop("siren_initialize", True)
...
def _spline_out_dims(what, shape_info, transformer_kwargs={}, num_bins=16):
...
def _bspline_out_dims(what, shape_info, transformer_kwargs={}, num_bins=32):
    #input for conditioner
    dim_out = 2 * num_bins * shape_info.dim_all(what) + 6 * shape_info.dim_noncircular(what)
    return {"params_net": dim_out}
...
CONDITIONER_OUT_DIMS = {
    ...
    bg.ConditionalBSplineTransformer: _bspline_out_dims
}
